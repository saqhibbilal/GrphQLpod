...
const investorQueries = {
  ...
  getById: (id) => database.get('SELECT * FROM investors WHERE id = $1', [id]),
  getByStartupId: (startupId) => database.queryAll(`
    SELECT i.* FROM investors i
    JOIN investor_startups ins ON i.id = ins.investor_id
    WHERE ins.startup_id = $1
  `, [startupId])
};

// Funding round queries
const fundingRoundQueries = {
  getByStartupId: (startupId) => database.queryAll('SELECT * FROM funding_rounds WHERE startup_id = $1', [startupId])
};
